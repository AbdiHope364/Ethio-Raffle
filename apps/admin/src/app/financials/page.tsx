  ...
  return (
    <div className="space-y-8 transition-colors">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 space-y-2 shadow-xs transition-colors">
          ...
          <div className="text-2xl font-black text-slate-900 dark:text-white font-mono">
            {data?.summary?.totalGrossRevenue.toLocaleString() || "0"} <span className="text-sm font-bold text-slate-400">ETB</span>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 space-y-2 shadow-xs transition-colors">
          ...
          <div className="text-2xl font-black text-blue-700 dark:text-blue-400 font-mono">
            {data?.summary?.totalCommissionLiabilities.toLocaleString() || "0"} <span className="text-sm font-bold text-slate-400">ETB</span>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 space-y-2 shadow-xs transition-colors">
          ...
          <div className="text-2xl font-black text-purple-700 dark:text-purple-400 font-mono">
            {data?.summary?.totalFloatInCirculation.toLocaleString() || "0"} <span className="text-sm font-bold text-slate-400">ETB</span>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 space-y-2 shadow-xs transition-colors">
          ...
          <div className="text-2xl font-black text-emerald-700 dark:text-emerald-400 font-mono">
            {data?.summary?.netPlatformProfit.toLocaleString() || "0"} <span className="text-sm font-bold text-slate-400">ETB</span>
          </div>
          <span className="text-[11px] text-emerald-600 dark:text-emerald-400 font-semibold block">
      ...
      <div className="space-y-4">
        <div className="flex bg-slate-100 dark:bg-slate-900 p-1.5 rounded-2xl border border-slate-200 dark:border-slate-800 text-xs font-bold w-fit">
          <button
                    ...
                    data?.transactions?.map((tx: any) => (
                      <tr key={tx.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition">
                        ...
                        <td className="py-3.5 px-4">
                          <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 font-bold text-[10px]">
                            {tx.paymentMethod}
                          </span>
                        </td>
                        ...
                        <td className="py-3.5 px-4">
                          <span className="text-[10px] font-extrabold px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-500/30">
                            {tx.status}
                          </span>
                        </td>
                  ...
                  {data?.agentLedgers?.map((entry: any) => (
                    <tr key={entry.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition">
                      ...
                      <td className={`py-3.5 px-4 font-mono font-bold ${
                        entry.amount > 0 ? "text-emerald-700 dark:text-emerald-400" : "text-slate-700 dark:text-slate-300"
                      }`}>
