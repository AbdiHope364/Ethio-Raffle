    ...
    <div className="space-y-8 transition-colors">
                  ...
                  <tr key={r.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition">
                    <td className="py-3.5 px-4">
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <div>
                          <div className="font-extrabold text-slate-900 dark:text-white line-clamp-1">{r.title}</div>
                          <div className="text-[10px] text-slate-500 dark:text-slate-400">{r.prizeName}</div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-3.5 px-4">
                      {r.commitHash ? (
                        <span className="font-mono text-[10px] text-purple-700 dark:text-purple-300 bg-purple-50 dark:bg-purple-950/60 px-2 py-1 rounded border border-purple-200 dark:border-purple-800 block truncate max-w-[120px]" title={r.commitHash}>
      ...
      {isCreateOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs animate-in fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-6 relative max-h-[90vh] overflow-y-auto text-slate-900 dark:text-white transition-colors">
            ...
            <form onSubmit={handleCreateRaffle} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">Title (English)</label>
                  <input
                    type="text"
                    required
                ...
                <div className="space-y-1">
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">Title (Amharic - አማርኛ)</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">Prize Name</label>
                  <input
                    type="text"
                    required
              ...
              <div className="space-y-1">
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">Prize Photo URL</label>
                <input
                  type="url"
                  required
                  value={formData.prizeImage}
                  onChange={(e) => setFormData({ ...formData, prizeImage: e.target.value })}
                  className="w-full px-3 py-2 text-xs rounded-xl bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 focus:ring-2 focus:ring-purple-500 text-slate-900 dark:text-white font-mono"
                />
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="space-y-1">
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">Ticket Price (ETB)</label>
                  <input
                    type="number"
                    required
                ...
                <div className="space-y-1">
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">Total Ticket Cap</label>
                  <input
                    type="number"
                    required
                ...
                <div className="space-y-1">
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">Draw Duration (Days)</label>
                  <input
                    type="number"
                    required
              ...
              <div className="bg-purple-50 dark:bg-purple-950/60 p-3 rounded-xl border border-purple-200 dark:border-purple-800/80 text-xs text-purple-800 dark:text-purple-200 flex items-center gap-2">
