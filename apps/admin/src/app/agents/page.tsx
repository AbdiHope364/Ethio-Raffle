    ...
    <div className="space-y-8">
                ...
                agents.map((ag) => (
                  <tr key={ag.id} className="hover:bg-slate-800/40 transition">
                    <td className="py-3.5 px-4">
                      <div className="font-extrabold text-white">{ag.fullName}</div>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-[11px] border border-slate-700 transition inline-flex items-center gap-1"
                      >
                        <Sliders className="w-3 h-3 text-purple-400" /> Edit / KYC
                      </button>
                    </td>
                  </tr>
                ))
      ...
      {isOnboardOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs animate-in fade-in">
          <div className="bg-slate-900 rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl border border-slate-800 space-y-5 relative max-h-[90vh] overflow-y-auto text-white">
            ...
            <form onSubmit={handleOnboard} className="space-y-4">
              <div className="space-y-1">
                <label className="block text-xs font-bold text-slate-300">Full Name (KYC)</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="block text-xs font-bold text-slate-300">Phone Number (MSISDN)</label>
                  <input
                    type="tel"
                    required
                ...
                <div className="space-y-1">
                  <label className="block text-xs font-bold text-slate-300">National ID Reference</label>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="block text-xs font-bold text-slate-300">Business / Kiosk Name</label>
                  <input
                    type="text"
                    required
                ...
                <div className="space-y-1">
                  <label className="block text-xs font-bold text-slate-300">Region / Subcity</label>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="space-y-1">
                  <label className="block text-xs font-bold text-slate-300">Commission Rate (%)</label>
                  <input
                    type="number"
                    step="0.1"
                    required
                ...
                <div className="space-y-1">
                  <label className="block text-xs font-bold text-slate-300">Daily Sales Limit</label>
                  <input
                    type="number"
                    required
                ...
                <div className="space-y-1">
                  <label className="block text-xs font-bold text-slate-300">Initial Float (ETB)</label>
                  <input
                    type="number"
                    required
      ...
      {selectedAgent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs animate-in fade-in">
          <div className="bg-slate-900 rounded-3xl max-w-md w-full p-6 sm:p-7 shadow-2xl border border-slate-800 space-y-5 relative text-white">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div>
            ...
            <form onSubmit={handleUpdateAgent} className="space-y-4">
              <div className="space-y-1">
                <label className="block text-xs font-bold text-slate-300">Account Status</label>
                <select
                  value={editStatus}
                  onChange={(e) => setEditStatus(e.target.value)}
                  className="w-full px-3 py-2 text-xs rounded-xl bg-slate-950 border border-slate-800 focus:ring-2 focus:ring-purple-500 text-white font-bold"
                >
              ...
              <div className="space-y-1">
                <label className="block text-xs font-bold text-slate-300">Audit Reason / Justification</label>
                <input
                  type="text"
                  required
                  value={editReason}
