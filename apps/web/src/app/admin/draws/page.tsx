  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      <div className="text-center space-y-3">
        <div className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full bg-purple-100 text-purple-800 text-xs font-bold border border-purple-200">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-sm space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
          <div className="md:col-span-6 space-y-2">
            ...
            <select
              ...
              disabled={isDrawing}
              className="w-full px-3 py-2.5 text-sm rounded-xl border border-slate-200 focus:ring-2 focus:ring-purple-500 bg-white font-bold"
            >
              {raffles.map((r) => (
          ...
          <div className="md:col-span-6 bg-slate-50 rounded-2xl p-4 border border-slate-200 text-xs space-y-1">
            <div className="flex items-center gap-1.5 font-bold text-slate-800">
              <Lock className="w-3.5 h-3.5 text-emerald-600" />
              <span>Published SHA-256 Pre-Commitment</span>
            </div>
            <p className="font-mono text-[10px] text-slate-500 truncate" title={selectedRaffle?.commitHash}>
        ...
        {selectedRaffle && (
          <div className="bg-gradient-to-br from-slate-950 via-slate-900 to-purple-950 text-white rounded-3xl p-6 sm:p-8 border border-slate-800 shadow-xl relative overflow-hidden space-y-6">
            ...
            {isDrawing && (
              <div className="bg-slate-900/90 rounded-2xl p-8 border border-purple-700/50 text-center space-y-4 animate-in fade-in">
                {countdown !== null ? (
            ...
            {drawResult && (
              <div className="bg-gradient-to-r from-purple-900/90 to-indigo-900/90 rounded-2xl p-6 sm:p-8 border border-purple-500 shadow-2xl space-y-6 animate-in fade-in zoom-in-95">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 pt-2">
                  ...
                  <Link
                    href={`/verifier?raffleId=${drawResult.raffle.id}&seed=${drawResult.raffle.revealedSeed}&commit=${drawResult.raffle.commitHash}&winner=${drawResult.winningTicketNumber}&total=${drawResult.raffle.totalTickets}&sold=${drawResult.raffle.soldTickets}`}
                  ...
                  >
                    <span>Run Public Verification Proof</span>
...
export default function AdminDrawsConsolePage() {
  return (
    <Suspense fallback={<div className="py-20 text-center text-xs text-slate-400">Loading draw console...</div>}>
