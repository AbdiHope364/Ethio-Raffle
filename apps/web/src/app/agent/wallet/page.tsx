...
export default function AgentWalletPage() {
  ...
  const getEntryBadge = (type: string) => {
    switch (type) {
      case "TOPUP":
        return <span className="bg-emerald-100 text-emerald-800 text-[10px] font-extrabold px-2 py-0.5 rounded flex items-center gap-1"><ArrowDownLeft className="w-3 h-3 text-emerald-600" /> TOPUP</span>;
      case "SALE_DEDUCTION":
        return <span className="bg-slate-100 text-slate-700 text-[10px] font-extrabold px-2 py-0.5 rounded flex items-center gap-1"><ArrowUpRight className="w-3 h-3 text-slate-500" /> SALE</span>;
      case "COMMISSION_ACCRUED":
        return <span className="bg-blue-100 text-blue-800 text-[10px] font-extrabold px-2 py-0.5 rounded flex items-center gap-1"><CheckCircle2 className="w-3 h-3 text-blue-600" /> COMMISSION</span>;
      case "PAYOUT":
        return <span className="bg-purple-100 text-purple-800 text-[10px] font-extrabold px-2 py-0.5 rounded flex items-center gap-1"><Wallet className="w-3 h-3 text-purple-600" /> PAYOUT</span>;
      default:
        return <span className="bg-slate-100 text-slate-700 text-[10px] font-extrabold px-2 py-0.5 rounded">{type}</span>;
    }
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm space-y-2">
          ...
          <div className="text-3xl font-black text-emerald-700 font-mono tracking-tight">
            {agent?.floatBalance ? agent.floatBalance.toLocaleString() : "0"} <span className="text-base text-slate-600">ETB</span>
          </div>
          <span className="text-[11px] text-slate-500 block">
            Mode: <strong className="text-slate-800">{agent?.walletMode || "PREPAID"}</strong>
          </span>
        </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm space-y-2">
          ...
          <div className="text-3xl font-black text-slate-900 font-mono tracking-tight">
            {agent?.dailySalesLimit ? agent.dailySalesLimit.toLocaleString() : "50,000"} <span className="text-base text-slate-600">ETB</span>
          </div>
          <span className="text-[11px] text-slate-500 block">
            ...
            <form onSubmit={handleTopup} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 text-sm font-mono font-bold rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-xl border text-xs font-bold flex items-center justify-center gap-1.5 ${
                      topupMethod === "CBE_BIRR"
                        ? "bg-emerald-50 text-emerald-800 border-emerald-600 ring-1 ring-emerald-600"
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 text-xs font-mono rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>
